...
The `macOS package` GitHub workflow runs this script on `macos-latest` for pull requests and manual dispatches when Actions is enabled. It is currently disabled manually during the private-repository quota-control period. When enabled, it uploads `macos-dmg` and `macos-sha256sum` artifacts for private review and does not bypass the public-release signing/notarization requirements below.
