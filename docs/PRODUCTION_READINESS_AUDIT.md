...
The repository has a solid production baseline, and the targeted hardening branches listed below have been integrated into local `main`. The app must not be treated as finished until the integrated `main` is pushed, local verification remains green, public signing/notarization is configured, and native Windows plus live credential-backed QA are completed.
...
These pull requests represented the production-hardening queue. Their branch commits have been integrated into local `main`; after pushing `main`, any GitHub PRs that remain open should be closed as merged or superseded rather than updated individually:
