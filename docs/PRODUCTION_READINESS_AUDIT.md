...
The repository has a solid production baseline, and the targeted hardening branches listed below have been integrated and pushed to `main`. The app must not be treated as finished until public signing/notarization is configured, native Windows plus live credential-backed QA are completed, and hosted or self-hosted protected checks can run without quota failures.
...
| Windows packaging | Existing Windows workflow files build MSI/NSIS artifacts and run MSI quiet install/uninstall smoke when Actions is enabled. The integrated packaging hardening adds pinned EUMDAC sidecar staging and packaged sidecar hash/signature verification. | GitHub Actions is currently disabled to avoid private-repository quota failures. Validate MSI and NSIS on real Windows 10/11 machines, including launch without dev tools, uninstall cleanup, Authenticode signature, and SmartScreen behavior. |
...
| GitHub, CI, and CD | Workflow files exist for manual-dispatch CI, macOS package, Windows package, Rust audit, and release publishing. Release secret preflight, release tag/version preflight, and version consistency guard tests are integrated. Open PR count is 0, the integrated `main` commit did not create new Actions runs, and `v2.1.1` is marked as Latest for the Tauri desktop app release line. | Workflows are currently `disabled_manually`, required status checks are temporarily removed from `main`, Dependabot version updates are paused, and Dependabot automated security fixes are disabled to avoid quota failure loops and email spam. Re-enable required checks only after free quota, public hosted runners, or self-hosted runners can pass reliably. Release secrets are not configured. |
...
- GitHub Actions hosted jobs are disabled because account billing or spending-limit state blocks runners and creates failed-check notification spam: #28.
...
- Required checks pass on `main` after GitHub Actions is re-enabled with free quota, a public repository, or self-hosted runners: Ubuntu verify, macOS verify, Windows verify, visual smoke, RustSec audit, macOS DMG build, and Windows MSI/NSIS build.
