...
GitHub and RustSec may report `glib` or unmaintained GTK3 bindings in
`Cargo.lock` because Tauri's Linux webview stack pulls `gtk`/`webkit2gtk` through
`wry`. The production release targets for this app are macOS and Windows, and
those bundles do not load the Linux GTK stack.
